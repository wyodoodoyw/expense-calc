...
async function getPairing(pairingNo) {
  const db = await openDB('PairingsDB', 1, {
    upgrade(db) {
      console.log('upgrade');
      const tx = db.transaction(['pairings'], 'readonly');
      const store = tx.objectStore('pairings');
      const val = store.get(pairingNo);
      console.log(`val: ${val}`);
    },
    blocked() {
      console.log(`blocked`);
...
const test = (pairingNo) => {
  try {
    console.log(`Testing...`);
    getPairing(pairingNo).then((pairing) => {
      console.log(`Test pairing ${pairingNo}: ${JSON.stringify(pairing)}`);
    });
  } catch {
    console.log(`Try/Catch Error in pairing `);
  }
