...
async function checkAllPairings(min, max, { logAll = false } = {}) {
  ...
  if (!pairings || pairings.length === 0) {
    ...
    return { total: 0, processed: 0, mismatches: 0 };
  }
...
export default function TestComponent(props) {
  ...
  const handleRunTests = async (e) => {
    ...
    try {
      const result = await checkAllPairings(min, max, { logAll });
      setResults(result);
    } catch (err) {
  ...
  return (
    <div>
      <LoadingIndicator loading={loading} message="Testing..." />
    </div>
  );
}
